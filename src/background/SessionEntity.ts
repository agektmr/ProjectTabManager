...
export class SessionEntity {
  ...
  constructor(target: chrome.windows.Window | SessionEntity) {
    ...
    Util.log(
      '[SessionEntity] Created a new session entity %o based on %o',
      this,
      target,
    );
  }
   ...
   */
  public addTab(tab: TabEntity | chrome.tabs.Tab): void {
   ...
   */
  public updateTab(tab: chrome.tabs.Tab | undefined): void {
  ...
  public removeTab(tabId: number): boolean {
    ...
    if (i === -1) return false;
    Util.log(
      '[SessionEntity] removed tab %d from session %s',
      this.tabs[i].id,
      this.id,
    );
    // Remove TabEntity
  ...
  public openTabs(lazify: boolean = false): Promise<void> {
      ...
      chrome.windows.create(
        ...
        (win) => {
          ...
          this.tabs.forEach((tab, i) => {
            ...
            let url = lazify
              ? tab.url
              : Util.lazify(tab.url, tab.title, tab.favIconUrl);
            chrome.tabs.create(
              {
                windowId: win?.id,
                index: i,
  ...
  public setId(projectId: string): void {
    this.id = projectId;
    Util.log(
      '[SessionEntity] Assigned project %s to session %o',
      projectId,
      this,
    );
  }
  ...
  public unsetId(): void {
    Util.log(
      '[SessionEntity] Removed project %s from session %o',
      this.id,
      this,
    );
    this.id = '';
  ...
  public setWinId(winId: number): void {
    this.winId = winId;
    Util.log(
      '[SessionEntity] Assigned window %s to session %o',
      this.winId,
      this,
    );
  }
  ...
  public unsetWinId(): void {
    Util.log(
      '[SessionEntity] Removed window %s from session %o',
      this.winId,
      this,
    );
    this.winId = undefined;
