...
export class PtmApp extends PtmBase {
  ...
  render() {
    return html` <style>
      ...
      <ptm-project-linker
        ...
        @unlink-project="${this.unlinkProject}"
      ></ptm-project-linker>
      <mwc-top-app-bar-fixed>
        ...
        <mwc-tab-bar id="tabs" @MDCTabBar:activated="${this.changeTab}">
      ...
      <mwc-snackbar id="toast" duration="3000" labelText="${this.toastText}">
      </mwc-snackbar>`;
  }
  ...
  private getProjectFromId(projectId: string): ProjectEntity | undefined {
  ...
  private async linkProject(e: CustomEvent): Promise<void> {
    try {
  ...
  private async unlinkProject(e: CustomEvent): Promise<void> {
    try {
  ...
  private async openProject(e: CustomEvent): Promise<void> {
    try {
  ...
  private async createProject(e: CustomEvent): Promise<void> {
    const project = this.getProjectFromId(e.detail.id);
    if (!project) {
      this.fire('show-toast', {
        // TODO: l10n-nize
  ...
  private async removeSession(e: CustomEvent): Promise<void> {
    const qs: PtmDialogQueryString = {
  ...
  private async renameProject(e: CustomEvent): Promise<void> {
    try {
      const project = this.getProjectFromId(e.detail.id);
      const qs: PtmDialogQueryString = {
  ...
  private async removeProject(e: CustomEvent): Promise<void> {
    try {
      const qs: PtmDialogQueryString = {
