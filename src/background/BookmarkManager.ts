...
export class BookmarkManager {
  ...
  static removeBookmark(bookmarkId: string): Promise<void> {
    return new Promise((resolve) => {
  ...
  static flat(
    ...
    // @ts-ignore
    return src.flatMap((node: chrome.bookmarks.BookmarkTreeNode) => {
  ...
  static getRootFolder(): Promise<chrome.bookmarks.BookmarkTreeNode> {
    return new Promise((resolve) => {
      chrome.bookmarks.getSubTree(
        ...
        (bookmarks) => {
          if (!bookmarks) throw '[BookmarkManager] Bookmark root not found.';
  ...
  static async getBookmarkFolders(): Promise<
    chrome.bookmarks.BookmarkTreeNode[]
  > {
  ...
  static async archiveFolder(bookmarkId: string): Promise<void> {
      ...
      chrome.windows.create(
        ...
        (win) => {
          ...
          for (let bookmark of bookmarks) {
            ...
            if (i++ === 0 || bookmark.url === undefined) continue;

            let url = lazify
              ? bookmark.url
              : Util.lazify(bookmark.url, bookmark.title);
            chrome.tabs.create({
              windowId: win.id,
              url: url,
              active: false,
  ...
  static openEditWindow(bookmarkId: string): void {
