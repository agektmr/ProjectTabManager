  ...
  SessionManager.prototype = {
    ...
    onattached: function(tabId, attachInfo) {
      ...
      getWindowInfo(attachInfo.newWindowId).then(function(win) {
        ...
        var session = that.getSessionFromWinId(attachInfo.newWindowId);
        // If this tab generates new window, it should be a new session
        if (!session) {
          session = new SessionEntity(win);
          that.sessions.unshift(session);
        }
    ...
    onactivated: function(activeInfo) {
      ...
      getWindowInfo(activeInfo.windowId).then(function(win) {
        if (win === undefined) return;
        that.activeInfo.tabId    = activeInfo.tabId; // not used
        that.activeInfo.windowId = activeInfo.windowId;
      });
    },
    ...
    onwindowcreated: function(win) {
      ...
      getWindowInfo(win.id).then(function(win) {
        // If this is a project intentionally opened
        if (that.openingProject) {
          if (config_.debug) console.log('[SessionManager] Intentionally opened window', win);
          var session = that.getSessionFromProjectId(that.openingProject);
          if (session) {
            ...
            session = new SessionEntity(win);
            that.sessions.unshift(session);
            session.setId(that.openingProject);
          }
          that.setActiveSession(win.id, session);
          that.openingProject = null;
        } else {
          ...
          for (var i = 0; i < that.sessions.length; i++) {
            if (that.sessions[i].winId) continue;
            if (that.compareTabs(win, that.sessions[i])) {
              // set project id and title to this session and make it bound session
              that.sessions[i].setWinId(win.id);
              if (config_.debug) console.log('[SessionManager] Associated with a previous session', that.sessions[i]);
              that.setActiveSession(win.id, that.sessions[i]);
              UpdateManager.storeSessions();
              return;
            }
          }
    ...
    resumeSessions: function(callback) {
      ...
      return new Promise(function(resolve, reject) {
        ...
        UpdateManager.restoreSessions(function(sessions) {
          ...
          sessions = that.cleanSessions(sessions);
          that.prev_sessions = sessions;
          chrome.windows.getAll({populate: true}, function(windows) {
            ...
            Array.prototype.forEach.call(windows, that.restoreSession.bind(that));
            ...
            for (i = 0; i < that.prev_sessions.length; i++) {
              var session = new SessionEntity(that.prev_sessions[i]);
              // `push` not `unshift`
              that.sessions.push(session);
              if (config_.debug) console.log('[SessionManager] This session window is not open.');
              that.prev_sessions.splice(i--, 1);
            }
